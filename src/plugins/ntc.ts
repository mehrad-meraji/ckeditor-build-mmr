...
const ntc = {
	init: function () {
		...
		for (let i = 0; i < ntc.names.length; i++) {
			color = "#" + ntc.names[i][0];
			rgb = ntc.rgb(color).toString();
			hsl = ntc.hsl(color).toString();
	...
	name: function (color: string) {
		...
		for (let i = 0; i < ntc.names.length; i++) {
			...
			ndf1 = Math.pow(r - Number(ntc.names[i][3]), 2) + Math.pow(g - Number(ntc.names[i][4]), 2) + Math.pow(b - Number(ntc.names[i][5]), 2);
			ndf2 = Math.abs(Math.pow(h - Number(ntc.names[i][6]), 2)) + Math.pow(s - Number(ntc.names[i][7]), 2) + Math.abs(Math.pow(l - Number(ntc.names[i][8]), 2));
			ndf = ndf1 + ndf2 * 2;
			if (df < 0 || df > ndf) {
				df = ndf;
				cl = i;
	...
	hsl: function (color: string): number[] {
		...
		function from255(n: number): string {
			const result = n * 255
			return result + "";
		}

		return [parseInt(from255(h)), parseInt(from255(s)), parseInt(from255(l))];
	},

	// adopted from: Farbtastic 1.2
	// http://acko.net/dev/farbtastic
	rgb: function (color: string): number[] {
		return [parseInt('0x' + color.substring(1, 3)), parseInt('0x' + color.substring(3, 5)), parseInt('0x' + color.substring(5, 7))];
	},

	shadeRgb: function (shadename: string): string {
